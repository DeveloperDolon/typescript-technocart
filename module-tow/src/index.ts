...
interface MyArray {
  // ------> array type declare with interfaces
  [index: number]: number;
}
interface StudentFunc {
  // ----> function type declare with interfaces
...
const dolon: Developer<
  AppleWatch,
  { name: string; price: number; ingine: string }
> = {
  name: "Dolon Chandra",
  age: 22,
  computer: {
...
const moneySending = sendMoney<number, Info>(343, {
  name: "Dolon",
  age: 34,
...
const areaInfo: AreaString = {
  // one way of making mapped types in typescript
  height: "233",
  name: "SOmeting",
  width: "433",
};
